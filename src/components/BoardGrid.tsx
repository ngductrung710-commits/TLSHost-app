  ...
  return (
    ...
    >
      <div
        ...
        // The bar shows a name; the tooltip carries the detail that will not
        // fit in a two-night bar. Both are also in the visually hidden summary
        // below the board, so this is convenience, not the only route to it.
        title={`${span.label} · ${span.nights} đêm${
  ...
  if (board.rooms.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-line-strong bg-surface p-10 text-center">
        <p className="text-[15px] font-semibold text-ink-900">Chưa có phòng nào</p>
        <p className="mx-auto mt-2 max-w-sm text-[14px] leading-relaxed text-ink-600">
          Bảng lịch cần ít nhất một phòng để có gì mà hiển thị. Thêm chỗ nghỉ đầu
          tiên rồi quay lại đây.
        </p>
        <Link
          href="/cho-nghi/moi"
          ...
          {board.days.map((day, i) => (
            <div
              ...
              className={[
                ...
                i === todayIndex ? "bg-clay-50" : isWeekend(day) ? "bg-sand-50" : "",
              ].join(" ")}
            >
          ...
          {board.rooms.flatMap((room) =>
            room.spans.map((span) => (
              <li key={`${room.id}-${span.kind}-${span.id}`}>
                {room.propertyName} — {room.name}: {span.label}, {span.nights} đêm
                {span.source ? `, ${SOURCE_LABELS[span.source] ?? span.source}` : ""}
              </li>
            )),
          )}
