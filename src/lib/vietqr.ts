...
export type VietQrInput = {
  ...
  memo?: string;
  /**
   * Beneficiary name and city, tags 59 and 60.
   *
   * EMVCo marks both mandatory for a merchant-presented code, and the one
   * real-world sample available here — a VietQR generated by TPBank's own app
   * for this very account — carries both. The first version of this file left
   * them out and nothing complained, because nothing here can complain: only a
   * banking app can, and it does so by refusing to scan.
   *
   * Optional in this signature rather than required, because a caller with no
   * city should send a QR without one instead of inventing "Ha Noi".
   */
  merchantName?: string;
  merchantCity?: string;
};
...
export function vietQrPayload({
  ...
  // Both are stripped the same way the memo is: a bank that rejects diacritics
  // in a transfer description rejects them in a merchant name too, and "NGUYỄN
  // ĐỨC TRUNG" is exactly the value someone will paste in one day.
  if (merchantName !== undefined) {
    const name = sanitiseMemo(merchantName);
    if (name !== "") payload += tlv(TAG.MERCHANT_NAME, name);
  }
  if (merchantCity !== undefined) {
    const city = sanitiseMemo(merchantCity).slice(0, 15);
